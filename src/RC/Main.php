...
class Main extends PluginBase implements Listener{
	...
	public $prefix = "§7[§4System§7]";
	public $reportPrefix = "§7[§bReportSystem§r§7]";
	...
	public function onEnable(){
		$oldreports = scandir("/root/Server/ReportCore/Reports/");
		...
		public function onJoin(PlayerJoinEvent $event)
	{
		...
		$c = new Config("/root/Server/ReportCore/$pname.yml");
		$c->set("Reporter", null);
		$c->set("Reported", null);
		$c->set("Grund", null);
		$c->set("Status", "Offen");
		$c->set("Staff", null);
		$c->set("Port", $this->getServer()->getPort());
		$c->save();
		}
			...
			if(strtolower($cmd) == "notify"){
				if($sender instanceof Player){
					if($sender->hasPermission("rc.notify")){
						$tcfg = new Config("/root/Server/ReportCore/Notify/".$sender->getName().".yml", Config::YAML);
...
$tcfg->save();
				...
				if(strtolower($cmd == "report")){
					...
					if(empty($args[0]) || empty($args[1])){
						...
						if(file_exists("/root/Server/ReportCore/Spieler/".$args[0].".yml")){
							$tconfig = new Config("/root/Server/ReportCore/Jump/".$args[0].".yml", Config::YAML);
							if($tconfig->get("Status") == "Online"){
								if($args[1] == "Hacking" || $args[1] == "Teaming" || $args[1] == "Bugusing" || $args[1] == "Wortwahl" || $args[1] == "Werbung" || $args[1] == "Sonstiges"){
									$reports = 0;
									$rcfg = new Config("/root/Server/ReportCore/Reports/".self::randomString().".yml", Config::YAML);
									$rcfg->set("Reporter", $sender->getName());
									$rcfg->set("Reported", $args[0]);
									$rcfg->set("Grund", $args[1]);
									$rcfg->set("Status", "Offen");
									$rcfg->set("Staff", null);
									$rcfg->set("Port", $this->getServer()->getPort());
									$rcfg->save();
									...
									foreach($this->getServer()->getOnlinePlayers() as $p){
										if(file_exists("/root/Server/ReportCore/Notify/".$p->getName().".yml")){
										$tcfg = new Config("/root/Server/ReportCore/Notify/".$p->getName().".yml", Config::YAML);
										if($tcfg->get("Notify") == true){
		...
		if(strtolower($cmd == "current")){
			if($sender instanceof Player){
				if($sender->hasPermission("rc.current")){
					...
					$jumper1 = new Config("/root/Server/ReportCore/Notify/".$sender->getName().".yml", Config::YAML);
					if($jumper1->get("Report") != null){
						$reportid = $jumper1->get("Report");
						$reportconfig = new Config("/root/Server/ReportCore/Reports/".$reportid.".yml", Config::YAML);
						switch($args[0]){
							case "close":
								unlink("/root/Server/ReportCore/Reports/".$reportid.".yml");
	...
	if(strtolower($cmd == "rpurge")){
		if($sender instanceof Player){
			if($sender->hasPermission("rc.rpurge")){
				unlink("/root/Server/ReportCore/Reports/*.*");
		...
		if(strtolower($cmd == "claim")){
			if($sender instanceof Player){
				if($sender->hasPermission("rc.notify")){
					if(file_exists("/root/Server/ReportCore/Reports/".$args[0].".yml")){
						$report = new Config("/root/Server/ReportCore/Reports/".$args[0].".yml", Config::YAML);
						$jumper = new Config("/root/Server/ReportCore/Notify/".$sender->getName().".yml", Config::YAML);
...
$config = new Config("/root/Server/ReportCore/Reports/".$args[0].".yml", Config::YAML);
...
$stringer = str_replace(":als", $report->get("Reported"), $commander);
			...
			if(strtolower($cmd == "jumpto")){
				if($sender instanceof Player){
				   if(empty($args[0])){
					if($sender->hasPermission("rc.jumpto")){
						if(file_exists("/root/Server/ReportCore/Reports/".$args[0].".yml")){
							$jump = new Config("/root/Server/ReportCore/Spieler/".$args[0].".yml", Config::YAML);
							$port = $jump->get("Port");
							
							$sender->transfer("0.0.0.0", $port);
						}else{
							$sender->sendMessage($this->reportPrefix ."§cDieser Spieler existiert nicht");
						}
					}
				}
			}
			if(strtolower($cmd == "reports")){
				if($sender instanceof Player){
					if($sender->hasPermission("rc.notify")){
						$files = scandir("/root/Server/ReportCore/Reports");

						foreach($files as $report){
							$filename = str_replace(".yml", "", $report);
								if($filename != "." && $filename != ".."){
									$configr = new Config("/root/Server/ReportCore/Reports/".$filename.".yml", Config::YAML);
...
public function onPacketR(DataPacketReceiveEvent $event){
		...
		if($event instanceof ModalFormResponsePacket){
			if($packet->formId == "7"){
				$reportc = new Config("/root/Server/ReportCore/Reports/".$data.".yml", Config::YAML);
				$reportc->set("Staff", $name);
				$reportc->set("Status", "Checking");
				$reportc->save();
				$staffconfig = new Config("/root/Server/ReportCore/Notify/".$name.".yml", Config::YAML);
...
class Check extends Task {
    ...
    public function onRun($tick) {
        $new  = scandir("/root/Server/ReportCore/Reports/");
        $tobc = array_diff($new, $this->oldreports);
        if ($tobc !== []) {
            foreach ($tobc as $repfile) {
                $rep = yaml_parse_file("/root/Server/ReportCore/Reports/" . $repfile);
